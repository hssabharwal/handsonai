---
title: Gemini Resources
description: Recommended docs, tools, courses, and community links for Google Gemini
---Recommended articles, docs, and courses for Google's Gemini models.

## Get Started

| Resource | Description |
|----------|-------------|
| [gemini.google.com](https://gemini.google.com/) | Gemini web app |
| [Download Gemini](https://gemini.google.com/app) | Mobile app (iOS, Android) |
| [Gemini FAQ](https://gemini.google.com/faq) | What Gemini can do and how to access it |
| [Pricing](https://one.google.com/about/ai-premium) | Plans: Free, Google AI Pro, Google AI Ultra |

## Learn

| Resource | Description |
|----------|-------------|
| [Google AI for Developers](https://ai.google.dev/) | Developer hub for Gemini API, Gemma, and more |
| [AI Studio Quickstart](https://ai.google.dev/gemini-api/docs/ai-studio-quickstart) | Get started with Google AI Studio |
| [Gemini API Cookbook](https://github.com/google-gemini/cookbook) | Tutorials, quick starts, and practical examples |
| [Gemini 101 Learning Path](https://developers.google.com/learn/pathways/solution-ai-gemini-101) | Step-by-step guide using AI Studio and Node.js |

## Documentation

| Resource | Description |
|----------|-------------|
| [Gemini API Docs](https://ai.google.dev/gemini-api/docs) | API reference and developer guides |
| [Gemini Models](https://ai.google.dev/gemini-api/docs/models) | Available models and capabilities |
| [Gemini Apps Help Center](https://support.google.com/gemini/) | FAQs, tips, and troubleshooting |
| [API Changelog](https://ai.google.dev/gemini-api/docs/changelog) | API updates and release notes |
| [App Release Notes](https://gemini.google/release-notes/) | Gemini app updates and improvements |

## Developer Tools

| Resource | Description |
|----------|-------------|
| [Google AI Studio](https://ai.google.dev/aistudio) | Browser-based prompt prototyping and API code generation |
| [Google Gemini on GitHub](https://github.com/google-gemini) | Official repositories (CLI, cookbook, SDKs) |
| [Gemini CLI](https://github.com/google-gemini/gemini-cli) | Open-source AI agent for the terminal |
| [Gemini Code Assist](https://codeassist.google/) | AI coding assistant for IDEs and GitHub |
| [Vertex AI](https://cloud.google.com/vertex-ai) | Enterprise AI platform with Gemini models |

## Stay Updated

| Resource | Description |
|----------|-------------|
| [Google DeepMind Blog](https://deepmind.google/blog/) | AI breakthroughs, projects, and updates |
| [Google AI Blog](https://blog.google/technology/ai/) | Product announcements and launches |
| [YouTube — Google DeepMind](https://www.youtube.com/@Google_DeepMind) | Research talks, demos, and documentaries |
| [@GoogleDeepMind on X](https://x.com/GoogleDeepMind) | Research updates |
| [@GeminiApp on X](https://x.com/GeminiApp) | Product updates and tips |
| [Gemini Apps Community](https://support.google.com/gemini/community) | User forum for questions and feedback |
| [Gemini CLI Discussions](https://github.com/google-gemini/gemini-cli/discussions) | Developer community for Gemini CLI |

## Gemini Enterprise

| Resource | Description |
|----------|-------------|
| [What is Gemini Enterprise?](https://docs.cloud.google.com/gemini/enterprise/docs) | Overview — intranet search, AI assistant, and agentic platform for organizations |
| [Get Started with Gemini Enterprise](https://docs.cloud.google.com/gemini/enterprise/docs/quickstart-gemini-enterprise) | Quickstart tutorial to create your first Gemini Enterprise app |
| [Compare Editions](https://docs.cloud.google.com/gemini/enterprise/docs/editions) | Feature comparison across Standard, Plus, and Frontline editions |
| [Gemini Enterprise Resources Hub](https://cloud.google.com/gemini-enterprise/resources) | Admin support, setup guides, and documentation by edition |
| [Workspace with Gemini (Admin)](https://support.google.com/a/answer/13623623) | Admin guide for Gemini in Google Workspace (Business/Enterprise) |
| [Gemini for Workspace FAQ](https://support.google.com/a/answer/14130944) | Frequently asked questions about Gemini in Workspace apps |
| [Gemini AI in Workspace](https://support.google.com/a/answer/15756885) | How Gemini features are included in Workspace subscriptions |
| [What is NotebookLM Enterprise?](https://docs.cloud.google.com/gemini/enterprise/notebooklm-enterprise/docs/overview) | AI-powered research tool for enterprise — compliant, admin-managed |
| [Set Up NotebookLM Enterprise](https://docs.cloud.google.com/gemini/enterprise/notebooklm-enterprise/docs/set-up-notebooklm) | Admin setup guide for authentication, IAM roles, and data residency |
| [Release Notes](https://docs.cloud.google.com/gemini/enterprise/docs/release-notes) | Latest updates including Gemini 3 Flash, Agent Designer, and semantic search |

## Gemini in Chrome

| Resource | Description |
|----------|-------------|
| [Gemini in Chrome](https://gemini.google/overview/gemini-in-chrome/) | Official overview — AI assistance right in your browser |
| [Chrome AI Innovations](https://www.google.com/chrome/ai-innovations/) | Feature showcase for Gemini-powered Chrome capabilities |
| [Chrome Gets Gemini 3 Features](https://blog.google/products-and-platforms/products/chrome/gemini-3-auto-browse/) | Google blog — side panel, auto browse, image generation, and more |
| [New AI Features for Chrome](https://blog.google/products/chrome/new-ai-features-for-chrome/) | Earlier Chrome AI features including tab organization and writing help |

## Research & Safety

| Resource | Description |
|----------|-------------|
| [Google DeepMind Research](https://deepmind.google/research/) | AI research publications |
| [Responsibility & Safety](https://deepmind.google/about/responsibility-safety/) | AI safety principles and practices |
| [Google Workspace Status](https://www.google.com/appsstatus/dashboard/) | Service status dashboard (includes Gemini) |
