...
Start with the **Analyze** skill (Step 1 of the framework). You'll repeat these steps for each skill you want to add.

1. Open the [Analyze SKILL.md file](https://github.com/jamesgray-ai/handsonai-plugins/blob/main/plugins/business-first-ai/skills/analyze/SKILL.md) in your web browser
2. Click the **Raw** button (top-right of the file) to see the plain text
3. Select all the text (**Ctrl+A** on Windows, **Cmd+A** on Mac) and copy it (**Ctrl+C** / **Cmd+C**)

**Step 3 — Create the skill folder**

1. Open your **Desktop** (or any folder you like)
2. Create a **new folder** and name it `analyze`
3. Open the `analyze` folder
4. Open a text editor (Notepad on Windows, TextEdit on Mac)
   - **TextEdit users:** before pasting, go to **Format** → **Make Plain Text** so it saves as plain text, not rich text
5. Paste the text you copied (**Ctrl+V** / **Cmd+V**)
6. Save the file **inside the `analyze` folder** with the name `SKILL.md`
   - **Notepad users:** in the "Save as type" dropdown, choose **All Files** so it doesn't add `.txt` to the end

When you're done, you should have: `analyze` → `SKILL.md` (one folder containing one file).
...
Zip the **folder itself** — not the file inside it. When you open the ZIP, you should see a single folder named `analyze`, not a loose `SKILL.md` file.
:::

**Step 5 — Upload the skill to Claude**
...
**Step 8 — Add more skills as you need them**

Go back to Step 2 and repeat for the next skill you need. Here are the direct links to each skill file:

| Framework Step | Skill | File |
|---|---|---|
