...
:::note[Prerequisites]
These skills work best with a **Notion account** and the **Notion MCP connector**. Without it, Claude can follow the naming and documentation conventions but cannot save entries to Notion. See [Notion Registry Setup](../../../builder-setup/notion-registry-setup/) for configuration instructions.
:::
## Get These Skills
...
Works with **Claude Code, Cursor, Codex CLI, Gemini CLI, VS Code Copilot**, and more. See [How to Add Skills to Your Platform](../../../agentic-building-blocks/skills/#how-to-add-skills-to-your-platform) for step-by-step instructions for each tool.
...
One-command install in the Claude Code CLI with automatic slash commands:

```bash
/plugin install ai-registry@handsonai
```

See [Using Plugins](../using-plugins/) for setup details, including [Claude.ai upload](../using-plugins/#using-skills-in-claudeai-web), [Cowork](../using-plugins/#using-plugins-in-claude-cowork), and [API usage](../using-plugins/#using-skills-via-the-claude-api).
  </TabItem>
</Tabs>
...
Claude will still follow the naming conventions, SOP templates, and documentation patterns — it just won't save to Notion. See [Notion Registry Setup](../../../builder-setup/notion-registry-setup/) to configure the MCP connector.
