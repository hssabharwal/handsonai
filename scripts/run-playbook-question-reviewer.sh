#!/bin/bash
# Wrapper script for playbook-question-reviewer subagent
...
LOG_FILE="$LOG_DIR/playbook-question-reviewer_$TIMESTAMP.log"

echo "=== Playbook Question Reviewer ===" >> "$LOG_FILE"
...
/Users/jamesgray/.local/bin/claude --agent playbook-question-reviewer -p "Review all in-progress question drafts from the Notion Questions database" --dangerously-skip-permissions --verbose >> "$LOG_FILE" 2>&1
